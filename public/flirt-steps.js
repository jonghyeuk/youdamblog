...
const reactions=[
 {icon:'✨',label:'대화가 이어졌다',text:'상대가 웃음과 함께 질문을 돌려보냈어. 대화를 이어갈 여지가 보여.'},
 {icon:'🙂',label:'짧게 반응했다',text:'상대가 짧게 답했지만 새로운 질문은 없었어. 지금은 속도를 살필 때야.'},
 {icon:'⌛',label:'답장이 멈췄다',text:'메시지는 읽었지만 답장은 오지 않았어. 더 보내기보다 선택을 존중해야 해.'}
];
const reaction=()=>reactions[Math.max(followup,0)];
...
renderFlirt=function(){
 ...
 else if(flirtStep===4)flirtBody.innerHTML=`<span class="kicker">STEP 05 · 대화의 템포</span><h2>내 말에 반응이 왔어.<br>지금 속도는 어때?</h2><div class="timing-chat"><div class="sent">${fs().openers[opening][0]}</div><div class="received">${fs().replies[opening]}</div><div class="sent">${choices()[followup][0]}</div><div class="received"><b>${reaction().icon} ${reaction().label}</b><br>${reaction().text}</div><small>상대의 반응을 보고 다음 행동을 골라봐</small></div><div class="strategy-options">${paceOptions.map((x,i)=>`<button class="strategy-option ${paceChoice===i?'selected':''}" data-pace="${i}"><span>${x.icon}</span><div><b>${x.title}</b><small>${x.copy}</small></div><em>${i===0?'대화 유지':i===1?'여유 두기':'과속 주의'}</em></button>`).join('')}</div><div class="micro-guide"><b>맥락 포인트</b><p>${followup===0?'질문이 돌아왔으니 같은 속도로 가볍게 이어가도 좋아.':'지금은 새 메시지를 밀어붙이기보다 상대가 선택할 시간을 주는 편이 자연스러워.'}</p></div><button class="next flirt-next" data-flirt-advance>맥락에 맞는 약속 고르기 →</button>`;
 else if(flirtStep===5)flirtBody.innerHTML=`<span class="kicker">STEP 06 · 자연스러운 약속</span><h2>처음 만난 이유를<br>다음 만남의 명분으로.</h2><p>${fs().title}의 대화에서 실제로 이어질 법한 제안을 골라봐.</p><div class="plan-board"><header><small>STORY SO FAR</small><b>${fs().emoji} ${fs().title}</b><span>${reaction().icon} ${reaction().label} · ${paceOptions[paceChoice].title}</span></header><div>${plans().map((x,i)=>`<button class="plan-card ${planChoice===i?'selected':''}" data-plan="${i}"><span>${x[0]}</span><small>PLAN ${String(i+1).padStart(2,'0')}</small><h3>${x[1]}</h3><p>${x[2]}</p><em>${i===0?'맥락 연결':i===1?'가벼운 선택':'짧은 약속'}</em></button>`).join('')}</div></div><div class="proposal-preview"><small>보낼 메시지 미리보기</small><strong>“${plans()[planChoice][2]}”</strong><p>${fs().where}에서 생긴 공통 맥락을 그대로 이어가는 제안</p></div><button class="next flirt-next" data-flirt-advance>최종 성사 확률 보기 →</button>`;
