...
let affairStep=0,affairProfile=0,affairScene=0,affairChoice=-1,affairPoints=0,affairHistory=[];
...
function resetAffair(){affairStep=0;affairProfile=0;affairScene=0;affairChoice=-1;affairPoints=0;affairHistory=[]}
function affairProbability(){return Math.min(96,Math.max(6,12+ap().bias+as().bias+affairPoints*8))}
function affairTraits(){const risk=affairHistory.filter(x=>x==='risk').length,grey=affairHistory.filter(x=>x==='grey').length,safe=affairHistory.filter(x=>x==='safe').length;return{novelty:Math.min(98,30+risk*17+grey*8),validation:Math.min(98,32+risk*13+grey*11),secrecy:Math.min(98,18+risk*19+grey*10),responsibility:Math.min(98,35+safe*16-grey*3-risk*7)}}
function renderAffair(){
 ...
 if(affairStep===0)affairBody.innerHTML=`<div class="affair-opening"><span class="kicker">STEP 01 · 관계의 현재</span><small>당신의 바람기 상황 테스트</small><h2>설렘보다 먼저,<br><em>지금의 약속</em>을 봅니다.</h2><p>실제 외도를 예측하거나 사람을 진단하지 않습니다. 관계의 경계가 흔들리는 가상의 장면에서 내가 어떤 선택을 하는지 살펴보는 게임입니다.</p></div><div class="affair-profiles">${affairProfiles.map((x,i)=>`<button class="${affairProfile===i?'selected':''}" data-affair-profile="${i}"><span>${x.icon}</span><b>${x.title}</b><small>${x.copy}</small></button>`).join('')}</div><button class="next affair-next" data-affair-next>유혹의 장면 만나기 →</button>`;
